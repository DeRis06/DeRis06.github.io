...
class skillController extends Controller
{
    public function index(){
        $skill_icon = public_path('admin/devicon.json');
        $skill_data = file_get_contents($skill_icon);
    ...
    public function update(Request $request){
        if($request->method() == 'POST'){
            $request->validate([
                ...
                '_language.required'=>'wajib di isi',
                '_workflow.required'=>'wajib di isi',
            ]);
            ...
            return redirect()->route('skill.index')->with('success', 'Successfully added/created');
        }
    }
}
